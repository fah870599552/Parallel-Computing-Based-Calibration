...
class Mopso:
	...
	def evaluation_fitness(self):
		#计算适应值
		fitness_curr = fitness_(self.in_)
		#for i in range((self.in_).shape[0]):
			#fitness_curr.append(fitness_(self.in_[i]))#返回了一个list#这里是一个循环，每个粒子循环放入
	...
	def initialize(self):
		...
		#初始化个体最优
		self.in_p,self.fitness_p = init.init_pbest(self.in_,self.fitness_)#输出的一个最好的，回溯，某一代的最优
		#初始化外部存档
		self.archive_in,self.archive_fitness = init.init_archive(self.in_,self.fitness_)
		#初始化全局最优
		self.in_g,self.fitness_g = init.init_gbest(self.archive_in,self.archive_fitness,self.mesh_div,self.min_,self.max_,self.particals)#全局最优函数回溯
	def update_(self):
		...
		self.evaluation_fitness()#这里输出了什么呢，是所有粒子的适应度集合嘛
